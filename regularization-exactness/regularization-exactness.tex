...
the binary is nearly aligned with the flight direction --- and a crude but sufficient dispersal
...
\begin{equation}
  ...
  \qquad
  \int_S|\vec y_1||\vec y_2|\,\dd^2b\;\le\;C\,\frac{\eta^{3/2}}{\sin^{3}\theta}\, .
  \label{eq:seq}
\end{equation}
...
\end{equation}
For the mixed term the crude cap $|\vec y_1|\le C/b_{90}$ would lose a factor $\eta^{-1/2}$;
instead use that on $S$ the exit angle equals the geometric angle $\Theta_{\rm geo}$
($\sin\Theta_{\rm geo}=\sin\theta$) up to $\order{s/r}$, so the body--1 kick is pinned by
Lemma~\ref{lem:kick} at $\beta_1\simeq b_{90}\cot(\Theta_{\rm geo}/2)$:
$|\vec y_1|\le C\sin(\Theta_{\rm geo}/2)/b_{90}$. Since $\sin(\Theta/2)\ge\tfrac12\sin\Theta$ on
$(0,\pi)$, the dyadic term of the mixed integral is
\begin{equation*}
  \frac{C\sin(\Theta_{\rm geo}/2)}{b_{90}}\cdot\frac{C}{s}\cdot
  \frac{C\,b_{90}^2\,s^2}{r^2\sin^4(\Theta_{\rm geo}/2)}
  \;=\;\frac{C\,b_{90}\,s}{r^2\sin^3(\Theta_{\rm geo}/2)}
  \;\le\;\frac{C\,b_{90}\,s}{r^2(\sin\theta/2)^3}\,,
\end{equation*}
and the dyadic sum over $s\le\rho$ is geometric, dominated by $s=\rho$:
$\int_S|\vec y_1||\vec y_2|\le C\,b_{90}\rho/(r^2\sin^3\theta)\le C\,\eta^{3/2}/\sin^3\theta$
(no logarithm). The deep core
$s<b_{90}$ contributes one more dyadic term with the capped kicks, within the stated bounds. Note
$\eta^{3/2}/\sin^3\theta\le\sqrt\eta/\sin\theta$ on $\sin\theta\ge\sqrt\eta$, so the mixed term
sits inside the $\order{\sqrt\eta/\sin\theta}$ error budget of \S\ref{sec:near-assembly} at every
generic orientation.
\end{proof}
...
$|\vec y_i|\le C/p_i$ whenever the pericenter $p_i$ w.r.t.\ body $i$ exceeds $C'b_{90}$;
...
ratios), and the osculating two--body kick of Lemma~\ref{lem:kick} gives $|\vec y_i|\le C/p_i$; on
the hard--pass set the uniform cap applies.
...
passes body $j$, at some pericenter $q\le r$. The passage deflects it --- in the osculating sense,
up to the same $\order{b_{90}\ln(1/\eta)}$ relabelings, absorbed into $w$ --- by the Rutherford
angle $\Theta(\beta)$, $\tan(\Theta/2)=b_{90,j}/\beta$, in the plane spanned by $\Vh$ and the
impact direction, after which the leg to body $i$ is again straight. Introduce polar coordinates
$\bv=\vec s_j^{\,\rm proj}+\beta\,\hat e(\varphi)$; the backscattered part $\Theta\ge\pi/2$ has
$\beta\le Cb_{90}$, hence measure $\le Cb_{90}^2\le Cw^2$, absorbed in the first term. For
$\Theta<\pi/2$ the transverse miss vector at body $i$ is
\begin{equation}
  \vec m(\beta,\varphi)=g(\beta)\,\hat e(\varphi)-\vec\Delta_\perp+\order{b_{90}\ln(1/\eta)},
  \qquad g(\beta)\equiv\beta-\ell\tan\Theta(\beta),
  \label{eq:exit-map}
\end{equation}
and $p_i\le s$ forces $|\vec m|\le w$. The one property that matters is the monotonicity of $g$:
since $\Theta'(\beta)<0$,
\begin{equation*}
  g'(\beta)\;=\;1-\ell\,\Theta'(\beta)\sec^2\!\Theta(\beta)\;\ge\;1 ,
\end{equation*}
so $g$ is strictly increasing and sweeps its range exactly once; it vanishes at the Einstein radius
$\beta_E\simeq\sqrt{2b_{90,j}\ell}\le C\rho$ (for small angles $g(\beta)\simeq\beta-2b_{90,j}\ell/\beta$:
rays inside the ring are bent past the axis, rays outside not enough). Counting in the coordinates
$(g,\varphi)$,
\begin{equation*}
...
intermediate passes below $\rho$ feed the target through compositions of Rutherford maps; each
...
with pericenter $\le Cb_{90}$ w.r.t.\ one of the bodies. Then every trajectory of $N$ (a) contains
a hard pass, and (b) exits its last passage below $\rho$ with speed $\le CV\eta^{1/4}$ relative to
the (slow) bodies. Consequently
\begin{equation}
...
$\order{\sqrt\eta\ln^4(1/\eta)/\lambda^2}$, and the $\theta$--averages by $\order{\eta\,{\rm polylog}}$.
\end{lemma}

\begin{proof}
\emph{(a) A hard pass is necessary.} Failure to escape forces the perturber's osculating energy
...
$\sum_{\rm passes}CGm_j/q_{\rm pass}\ge\tfrac12V^2$; since passes at $q\ge\rho$ contribute
$\le CV^2\sqrt\eta$ each and their number below any scale is controlled by the targeting count of
Lemma~\ref{lem:cone}(ii), some pass must have $q\le Cb_{90}\ln(1/\eta)$ --- a hard pass, up to the
logarithm which we absorb into the polylog of \eqref{eq:Nmeasure}.
...
\eqref{eq:Nmeasure} follows. The excision costs are then immediate: the excluded closed--form
weight is $\le\mu(N)\cdot\sup|\vec y|^2\le\mu(N)\cdot C/b_{90}^2$ at generic orientations, and
$\le\mu(N)\cdot C/(\lambda b_{90})^2$ on the annulus $\mathcal A_\lambda$; the aligned
configurations are confined to the cone, whose $\sin\theta\,\dd\theta$ measure is $\order\eta$.
\end{proof}
...
$\dd^2\beta_1/\beta_1^2$)
$+\;\order{\eta/\sin^2\theta}+\order{\sqrt\eta\,\ln^4(1/\eta)/\lambda^2}$ (the excised sets: on
$S$ both the true and the closed--form weights are $\le C\!\sum_{\rm dyadic}s^2/[r^2\sin^2(\Theta_{\rm
geo}/2)]=\order{\eta/\sin^2\theta}$ by the measure count of Lemma~\ref{lem:seq}; on $N$ the
...
\end{equation}
and likewise about body 2. With $K(\lambda)\le C\lambda^{-p}$, $p\ge2$
...
Lemma~\ref{lem:seq} ($\eta^{3/2}/\sin^3\theta\le\sqrt\eta/\sin\theta$ there): all within the error
already displayed. By Lemma~\ref{lem:iso} and the same
...
+\eta^{3/2}/\sin^3\theta}$\ (Lemmas \ref{lem:harmonic}, \ref{lem:seq})\\[2pt]
sequential and non--dispersing excisions & $\order{\eta/\sin^2\theta
+\sqrt\eta\,\ln^4(1/\eta)/\lambda^2}$\ (Lemmas \ref{lem:seq}, \ref{lem:nondisp})\\[2pt]
...
$\eta^{3/2}/\sin^3\theta\le\eta^{3(1/2-\alpha)}$); this proves the first display of
\eqref{eq:thm-inplane}.
...
near--aligned orientations (Lemma~\ref{lem:cone}), and the dispersal estimate that excises the
exchange--capture set (Lemma~\ref{lem:nondisp}).
...
$K(\lambda)\le C\lambda^{-p}$. The proof there is elementary calculus on explicit conic formulas;
...
\emph{(c) Uniform boundedness.} By the chain rule, every first derivative appearing in (1)--(2) is
a finite sum of products of explicit rational--algebraic expressions in $(e,\cosh H,\sinh H)$
divided by powers of the three quantities that can degenerate: $e-1\ge c\lambda^2$,
$\sqrt{e^2-1}\ge\lambda$, and $e\cosh H-1\ge e-1$. For fixed $e$, each expression has a finite
limit as $|H|\to\infty$, approached at rate $\order{1/\cosh H}$: the flow straightens to free
motion because the force decays as $d^{-2}$, integrably along the leg, and the same holds for the
variational (Jacobi) flow, whose fundamental solutions are explicit derivatives of
\eqref{eq:kepler-flow} with respect to $(e,H_0)$. Each expression is moreover jointly continuous in
