...
Two physical points the proof makes precise: (i) the recoil of the struck
...
O(b_90/r) because it is an area-preserving relabeling.
...
| `deflection_map_check.py` | the per-trajectory lemmas: recoil vs pinned cutoffs, the incoming deflection map b -> beta* (O(1) shift, area-preserving Jacobian at O(eta)), and the osculating-element kick fidelity (Sec. 4) | ~3 min |
